...
{																																\
	void *cuset_addr;																							\
	cuModuleGetGlobal((CUdeviceptr*)&cuset_addr, NULL, m_module->getHandle(), #symbol);						\
	T cuset_val = (val);																					\
...
{																													\
	void *cuget_addr;																				\
	cuModuleGetGlobal((CUdeviceptr*)&cuget_addr, NULL, m_module->getHandle(), #symbol);						\
...
{																											\
	void *cuvar_addr;																		\
	cuModuleGetGlobal((CUdeviceptr*)&cuvar_addr, NULL, m_module->getHandle(), #symbol);						\
...
#ifndef _M_X64 // x86
	asm("ld.global.ca.u32 %0, [%1];": "=r"(res) : "r"(p));
#else // x64
...
__device__ inline uint64 ldca(const uint64 *p) {
	uint64 res;
#ifndef _M_X64 // x86
	asm("ld.global.ca.u64 %0, [%1];": "=l"(res) : "r"(p));
#else // x64
...
__device__ inline void *ldca(void * const *p) {
	void *res;
#ifndef _M_X64 // x86
	asm("ld.global.ca.u64 %0, [%1];": "=r"(res) : "r"(p));
#else // x64
	asm("ld.global.ca.u64 %0, [%1];": "=l"(res) : "l"(p));
#endif
	return res;
...
inline uint max_prime_below(uint n, uint nb);
