...
{
	// Compile the kernel
	m_compiler.setSourceFile("src/rt/kernels/persistent_kdtree.cu");
	...
	// Create the taskData
	m_taskData.resizeDiscard(TASK_SIZE * (sizeof(TaskKdtree) + sizeof(int)));
...
//#endif
	...
	S64 kdtreeSize = align<S64, 4096>((S64)(m_numTris*Environment::GetSingleton()->GetFloat("PersistentKDTree.nodeRatio") * sizeof(CudaKdtreeNode)));
	m_nodes.resizeDiscard(kdtreeSize);
	m_nodes.setOwner(Buffer::Cuda, true); // Make CUDA the owner so that CPU memory is never allocated
	//m_nodes.clearRange32(0, 0, kdtreeSize); // Mark all tasks as 0 (important for debug)
	m_triWoop.resizeDiscard((S64)(m_numTris*Environment::GetSingleton()->GetFloat("PersistentKDTree.triRatio") * sizeof(Vec4f)));
	m_triIndex.resizeDiscard((S64)(m_numTris*Environment::GetSingleton()->GetFloat("PersistentKDTree.idxRatio") * (1+1) * sizeof(S32)));
...
void CudaPersistentKDTreeBuilder::updateConstants()
{
	...
	Environment::GetSingleton()->GetIntValue("PersistentKDTree.maxDepth", cudaEnv.optMaxDepth);

	Environment::GetSingleton()->GetFloatValue("PersistentKDTree.ci", cudaEnv.optCi);

	Environment::GetSingleton()->GetFloatValue("PersistentKDTree.ct", cudaEnv.optCt);

	Environment::GetSingleton()->GetIntValue("PersistentKDTree.triLimit", cudaEnv.triLimit);

	Environment::GetSingleton()->GetIntValue("PersistentKDTree.triMaxLimit", cudaEnv.triMaxLimit);

	Environment::GetSingleton()->GetIntValue("PersistentKDTree.popCount", cudaEnv.popCount);

	Environment::GetSingleton()->GetFloatValue("PersistentKDTree.granularity", cudaEnv.granularity);

	Environment::GetSingleton()->GetFloatValue("PersistentKDTree.failRq", cudaEnv.failRq);

	Environment::GetSingleton()->GetIntValue("PersistentKDTree.failureCount", cudaEnv.failureCount);
...
#else
	m_taskData.clearRange32(0, TaskHeader_Empty, TASK_SIZE * (sizeof(int)+sizeof(TaskKdtree))); // Mark all tasks as empty (important for debug)
#endif
...
void CudaPersistentKDTreeBuilder::printPool(TaskStackKdtree &tasks, int numWarps)
{
#ifdef LEAF_HISTOGRAM
	printf("Leaf histogram\n");
	unsigned int leafSum = 0;
	unsigned int triSum = 0;
	for(S32 i = 0; i <= Environment::GetSingleton()->GetInt("PersistentKDTree.triLimit"); i++)
	{
		printf("%d: %d\n", i, tasks.leafHist[i]);
	...
	FW::String state = sprintf("Kdtree Top = %d; Tri Top = %d; Warp counter = %d; ", tasks.nodeTop, tasks.triTop, tasks.warpCounter);
#ifdef COUNT_NODES
	...
	Debug << "\n\nTasks" << "\n";
	TaskKdtree* task = (TaskKdtree*)m_taskData.getPtr(TASK_SIZE*sizeof(int));
	int stackMax = 0;
	int maxTaskId = -1;
...
F32 CudaPersistentKDTreeBuilder::buildCuda()
{
	...
	// Set input.
	KernelInputKdtree& in = *(KernelInputKdtree*)m_module->getGlobal("c_kdtree_in").getMutablePtr();
	...
	RtEnvironment& cudaEnv = *(RtEnvironment*)m_module->getGlobal("c_env").getMutablePtr();
	float k1 = Environment::GetSingleton()->GetFloat("PersistentKDTree.depthK1");
	float k2 = Environment::GetSingleton()->GetFloat("PersistentKDTree.depthK2");
	float f1 = Environment::GetSingleton()->GetFloat("PersistentKDTree.failK1");
	float f2 = Environment::GetSingleton()->GetFloat("PersistentKDTree.failK2");
	cudaEnv.optMaxDepth  = int(k1 * log2((F32)m_numTris) + k2);
	cudaEnv.failureCount = int(f1 * cudaEnv.optMaxDepth + f2);
#ifndef BENCHMARK
	...
	in = *(KernelInputKdtree*)m_module->getGlobal("c_kdtree_in").getMutablePtr();
	...
	m_taskData.setRange(TASK_SIZE * sizeof(int), &all, sizeof(TaskKdtree)); // Set the first task

	// Set parent task header
	m_taskData.setRange(0, &all.unfinished, sizeof(int)); // Set the first task

	// Prepare the task stack
	TaskStackKdtree& tasks = *(TaskStackKdtree*)m_module->getGlobal("g_taskStackKdtree").getMutablePtr();
	tasks.header     = (int*)m_taskData.getMutableCudaPtr();
	tasks.tasks      = (TaskKdtree*)m_taskData.getMutableCudaPtr(TASK_SIZE * sizeof(int));
	tasks.nodeTop    = 1;
	tasks.triTop     = 0;
	tasks.top        = 0;
	tasks.bottom     = 0;
	...
	tKernel += kernel.launchTimed();

	/*CudaKernel kernelDealloc = m_module->getKernel("deallocFreeableMemory");
	F32 deallocTime = kernel.launchTimed(1,1);
	...
	tasks = *(TaskStackKdtree*)m_module->getGlobal("g_taskStackKdtree").getPtr();
	if(tasks.unfinished != 0 || tasks.top >= tasks.sizePool || tasks.nodeTop >= m_nodes.getSize() / sizeof(CudaKdtreeNode) || tasks.triTop >= m_triIndex.getSize() / sizeof(S32)) // Something went fishy
	{
		tKernel = 1e38f;
		fail("%d (%d x %d) (%d x %d) (%d x %d)\n", tasks.unfinished != 0, tasks.top, tasks.sizePool, tasks.nodeTop, m_nodes.getSize() / sizeof(CudaKdtreeNode), tasks.triTop, m_triIndex.getSize() / sizeof(S32));
	}
	...
	/*Debug << "\n\nKdtree" << "\n";
	CudaKdtreeNode* nodes = (CudaKdtreeNode*)m_nodes.getPtr();
...
void CudaPersistentKDTreeBuilder::trimBuffers()
{
	...
	// Compute and save fragmentation
	...
	m_nodes.resize((nN + nL) * sizeof(CudaKdtreeNode));
	m_triWoop.resize(tT*3*sizeof(float4));
	m_triIndex.resize(tT*3*sizeof(int));
#else
#ifdef DUPLICATE_REFERENCES
	m_nodes.resize(nN * sizeof(CudaKdtreeNode));
	m_triWoop.resize(tT*3*sizeof(float4) + nL*sizeof(float4));
	m_triIndex.resize(tT*3*sizeof(int) + nL*sizeof(int));
#else
	m_nodes.resize(nN * sizeof(CudaKdtreeNode));
	m_triIndex.resize(tT*sizeof(int) + nL*sizeof(int));
#endif
#endif
...
void CudaPersistentKDTreeBuilder::getStats(U32& nodes, U32& leaves, U32& emptyLeaves, U32& stackTop, U32& nodeTop, U32& tris, U32& sortedTris, bool sub)
{
	TaskStackKdtree tasks = *(TaskStackKdtree*)m_module->getGlobal("g_taskStackKdtree").getPtr();
...
void CudaPersistentKDTreeBuilder::getAllocStats(U32& numAllocs, F32& allocSum, F32& allocSumSquare)
{
	TaskStackKdtree tasks = *(TaskStackKdtree*)m_module->getGlobal("g_taskStackKdtree").getPtr();
...
void CudaPersistentKDTreeBuilder::getFragmentationStats(F32& fInt, F32& fExt)
{
	fInt = m_fragIntern;
	fExt = m_fragExtern;
}
...
void CudaPersistentKDTreeBuilder::setCircularMallocHeader(bool set, U32 ofs, U32 prevOfs, U32 nextOfs)
{
...
#else
	Vec4u header(type, prevOfs, nextOfs, 0);
	m_mallocData.setRange(ofs, &header, sizeof(Vec4u));
#endif
...
int CudaPersistentKDTreeBuilder::setDynamicMemory()
{
	int baseOffset = 0;
...
// Allocate first chunk of memory for the methods than do not support direct allocation
	...
	int memSize = int(m_trisCompactIndex.getSize()/sizeof(int));
	kernelCopy.setParams(
		m_trisCompactIndex.getCudaPtr(),
		baseOffset,
		memSize);
...
void CudaPersistentKDTreeBuilder::computeFragmentation()
{
	m_fragIntern = -1.f; // Fragmentation not measured
	m_fragExtern = -1.f; // Fragmentation not measured
...
#if (MALLOC_TYPE != CIRCULAR_MALLOC) && (MALLOC_TYPE != CIRCULAR_MALLOC_FUSED) && (MALLOC_TYPE != CIRCULAR_MULTI_MALLOC) && (MALLOC_TYPE != CIRCULAR_MULTI_MALLOC_FUSED)
	return;
#endif

	U32 numAllocs;
	F32 allocSum, allocSumSquare;
	getAllocStats(numAllocs, allocSum, allocSumSquare);

#ifdef CIRCULAR_MALLOC_CHECK_INTERNAL_FRAGMENTATION
	int numWarpsPerBlock = Environment::GetSingleton()->GetInt("SubdivisionRayCaster.numWarpsPerBlock");
	int numBlocksPerSM = Environment::GetSingleton()->GetInt("SubdivisionRayCaster.numBlockPerSM");
	int numThreads = numWarpsPerBlock*WARP_SIZE * NUM_SM*numBlocksPerSM;
	...
	m_fragIntern = sumInter / allocSum;
#endif
...
#if (MALLOC_TYPE == CIRCULAR_MALLOC) || (MALLOC_TYPE == CIRCULAR_MULTI_MALLOC)
#ifndef CIRCULAR_MALLOC_DOUBLY_LINKED
			next = *(U32*)(&heapBase[ofs+sizeof(F32)]);
#else
			next = *(U32*)(&heapBase[ofs+2*sizeof(F32)]);
#endif
	...
	if(sumFree == 0.f)
		...
		m_fragExtern = 1.f - maxFree / sumFree;
#endif
}

//------------------------------------------------------------------------

F32 CudaPersistentKDTreeBuilder::convertWoop()
{
	// Convert woop triangles
	CudaKernel kernel = m_module->getKernel("createWoop");

	kernel.setParams(
		m_trisCompact.getCudaPtr(),
		m_triWoop.getMutableCudaPtr(),
		m_numTris);

	F32 woopTime = kernel.launchTimed(m_numTris, Vec2i(256, 1));
