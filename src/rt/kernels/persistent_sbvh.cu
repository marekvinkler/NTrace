...
__device__ __forceinline__ int* getTriIdxPtr(int dynamicMemory, int tris, int triIdxCtr)
{
#ifndef MALLOC_SCRATCHPAD
#if SCAN_TYPE < 2
...
{
	void* alloc = malloc(4*tris*sizeof(int));
	//if(alloc == NULL)
	//	printf("Out of memory!\n");
	return ((CUdeviceptr)alloc) - g_heapBase;
}

//------------------------------------------------------------------------

__device__ __forceinline__ void freeBuffers(int dynamicMemory)
{
	free((void*)(g_heapBase+dynamicMemory));
}
#endif
...
{
	int* curMem = getTriIdxPtr(s_task[threadIdx.y].dynamicMemory, triEnd-triStart, triIdxCtr);
#ifndef MALLOC_SCRATCHPAD
...
__device__ bool taskDecideType(int tid, volatile TaskBVH* newTask)
{
	...
	if(taskTerminationCriteria(numLeft, numRight, newTask->bbox, newTask->bboxLeft, newTask->bboxRight, termCrit, leftLeaf, rightLeaf))
	{
		...
		*((int4*)&node->children) = make_int4(triStart, triEnd, parentIdx, 0);
		...
		int triIdx = createLeaf(tid, triOfs, (float*)c_bvh_in.trisOut, (int*)c_bvh_in.trisIndexOut, triStart, triEnd, (float*)c_bvh_in.tris, getTriIdxPtr(s_task[threadIdx.y].dynamicMemory, triEnd-triStart, triIdxCtr)); 
#else
		int triIdx = createLeafWoop(tid, triOfs, (float4*)c_bvh_in.trisOut, (int*)c_bvh_in.trisIndexOut, triStart, triEnd, (float4*)c_bvh_in.tris, getTriIdxPtr(s_task[threadIdx.y].dynamicMemory, triEnd-triStart, triIdxCtr));
#endif
		if(tid == 0)
	...
	else
	{
		...
		if(leftLeaf)
		{
			...
			*((int4*)&node0->children) = make_int4(triStart, triRight, nodeIdx, 0);
...
#ifndef MALLOC_SCRATCHPAD
#ifndef WOOP_TRIANGLES
			childLeft = createLeaf(tid, leftOfs, (float*)c_bvh_in.trisOut, (int*)c_bvh_in.trisIndexOut, triStart, triRight, (float*)c_bvh_in.tris, getTriIdxPtr(s_task[threadIdx.y].dynamicMemory, triEnd-triStart, triIdxCtr));
#else
			childLeft = createLeafWoop(tid, leftOfs, (float4*)c_bvh_in.trisOut, (int*)c_bvh_in.trisIndexOut, triStart, triRight, (float4*)c_bvh_in.tris, getTriIdxPtr(s_task[threadIdx.y].dynamicMemory, triEnd-triStart, triIdxCtr));
#endif
#else
#ifndef WOOP_TRIANGLES
			childLeft = createLeaf(tid, leftOfs, (float*)c_bvh_in.trisOut, (int*)c_bvh_in.trisIndexOut, 0, triLeft, (float*)c_bvh_in.tris, getTriIdxPtr(s_task[threadIdx.y].dynamicMemory, numLeft, triIdxCtr));
#else
			childLeft = createLeafWoop(tid, leftOfs, (float4*)c_bvh_in.trisOut, (int*)c_bvh_in.trisIndexOut, 0, triLeft, (float4*)c_bvh_in.tris, getTriIdxPtr(s_task[threadIdx.y].dynamicMemoryLeft, numLeft, triIdxCtr));
#endif
#endif
#endif
			...
			if(tid == 0)
			{
				freeBuffers(s_task[threadIdx.y].dynamicMemoryLeft);
#ifdef BVH_COUNT_NODES
				atomicAdd(&g_taskStackBVH.numLeaves, 1);
		...
		if(rightLeaf)
		{
			...
			*((int4*)&node1->children) = make_int4(triRight, triEnd, nodeIdx, 0);
...
#ifndef MALLOC_SCRATCHPAD
#ifndef WOOP_TRIANGLES
			childRight = createLeaf(tid, rightOfs, (float*)c_bvh_in.trisOut, (int*)c_bvh_in.trisIndexOut, triRight, triEnd, (float*)c_bvh_in.tris, getTriIdxPtr(s_task[threadIdx.y].dynamicMemory, triEnd-triStart, triIdxCtr));
#else
			childRight = createLeafWoop(tid, rightOfs, (float4*)c_bvh_in.trisOut, (int*)c_bvh_in.trisIndexOut, triRight, triEnd, (float4*)c_bvh_in.tris, getTriIdxPtr(s_task[threadIdx.y].dynamicMemory, triEnd-triStart, triIdxCtr));
#endif
#else
#ifndef WOOP_TRIANGLES
			childRight = createLeaf(tid, rightOfs, (float*)c_bvh_in.trisOut, (int*)c_bvh_in.trisIndexOut, 0, triRight, (float*)c_bvh_in.tris, getTriIdxPtr(s_task[threadIdx.y].dynamicMemoryRight, numRight, triIdxCtr));
#else
			childRight = createLeafWoop(tid, rightOfs, (float4*)c_bvh_in.trisOut, (int*)c_bvh_in.trisIndexOut, 0, triRight, (float4*)c_bvh_in.tris, getTriIdxPtr(s_task[threadIdx.y].dynamicMemoryRight, numRight, triIdxCtr));
#endif
#endif
#endif
			...
			if(tid == 0)
			{
				freeBuffers(s_task[threadIdx.y].dynamicMemoryRight);
#ifdef BVH_COUNT_NODES
				atomicAdd(&g_taskStackBVH.numLeaves, 1);
				//printf("Force Leaf right (%d, %d)\n", triStart, triEnd);
#endif
...
__device__ void taskChildTask(volatile TaskBVH* newTask)
{
	...
	if(tid == 0)
	{
		newTask->dynamicMemoryLeft = allocBuffers(newTask->triEnd-newTask->triStart);
		newTask->dynamicMemoryRight = allocBuffers(newTask->triEnd-newTask->triStart);
	}
#endif
	...
	//newTask->dynamicMemory = s_task[threadIdx.y].dynamicMemory;
...
//#pragma unroll 2 // OPTIMIZE: Is this beneficial?
	for(int i = 0; i < 2; i++)
	{
		...
		{
			int *orig = (int*)&g_redSplits[g_taskStackBVH.size];
...
#else // ENQUEUE_TYPE == 3
// Adds subtasks of a task into a global task queue
__device__ void taskEnqueueSubtasksCache(int tid, int taskIdx) // Must be __noinline__
{
...
#ifdef MALLOC_SCRATCHPAD
	// Free the memory
	freeBuffers(s_task[threadIdx.y].dynamicMemory);
#endif
...
__device__ void taskFinishBinning(int tid, int taskIdx, int countDown)
{
	...
	if(s_sharedData[threadIdx.y][0] == -1)
	{
		...
		if(tid < PLANE_COUNT)
		{
			...
			if(leftCnt == (triEnd - triStart) || rightCnt == (triEnd - triStart))
				cost = CUDART_INF_F;
			...
			if(__ffs(__ballot(red[tid] == cost)) == tid+1) // First thread with such condition, OPTIMIZE: Can be also computed by overwrite and test: better?
			{
				...
				s_task[threadIdx.y].dynamicMemoryLeft = allocBuffers(leftCnt);
				s_task[threadIdx.y].dynamicMemoryRight = allocBuffers(rightCnt);
#endif
			}
		}
		...
		{
			int triRight = triStart + (triEnd - triStart) / 2; // Force split on unsubdivided task
			...
			//if(tid == 0)
			//{
			//	s_task[threadIdx.y].dynamicMemoryLeft = allocBuffers(triRight-triStart);
			//	s_task[threadIdx.y].dynamicMemoryRight = allocBuffers(triEnd-triRight);
			//}
#endif
		}
		else
...
//#ifdef SPLIT_TEST
			if(tid == 0)
			{
				...
				{
					printf("Failed node in task %d (%d x %d)!\n", taskIdx, left->cnt, triEnd - triStart);
				}
				if(right->cnt == triEnd - triStart)
				{
					printf("Failed node in task %d (%d x %d)!\n", taskIdx, right->cnt, triEnd - triStart);
				}
			}
//#endif
...
__device__ void taskFinishAABB(int tid, int taskIdx, unsigned countDown)
{
	...
	{
		// Data arrays
		float* x = (float*)getPPSTrisPtr(s_task[threadIdx.y].dynamicMemory, end-start);
		float* y = (float*)getPPSTrisIdxPtr(s_task[threadIdx.y].dynamicMemory, end-start);
		float* z = (float*)getSortTrisPtr(s_task[threadIdx.y].dynamicMemory, end-start);
...
__device__ void trianglesPlanePosition(int triStart, int triEnd, const float4& plane, int& tb, int& tf)
{
	...
	for(int triPos = triStart; triPos < triEnd; triPos += step)
	{
		...
		int pos;
		if(s_task[threadIdx.y].depth < MAX_SBVH_DEPTH)
		{
		...
		{
			CudaAABB dummy;
			pos = getPlaneCentroidPosition(plane, v0, v1, v2, dummy);
		}
	...
	if(tid < PLANE_COUNT)
	{
		...
		for(;triPos < triLast; triPos++)
		{
			...
			*shared = *global;
			...
			{
				pos = getPlanePosition(plane, v0, v1, v2);
				//int pos = getPlanePosition(plane, *((float3*)&s_task[threadIdx.y].v0), *((float3*)&s_task[threadIdx.y].v1), *((float3*)&s_task[threadIdx.y].v2));
				getAABB(v0, v1, v2, tbox);
			}
			else
			{
...
__device__ int classifyTri(int tid, int subtask, int start, int end, volatile const float4& splitPlane)
{
	...
	if(tripos < end)
	{
		int triidx = getTriIdxPtr(s_task[threadIdx.y].dynamicMemory, end-start, s_task[threadIdx.y].triIdxCtr)[tripos]*3;
		...
		if(s_task[threadIdx.y].depth < MAX_SBVH_DEPTH)
		{
			pos = getPlanePosition(*((float4*)&splitPlane), v0, v1, v2);
		}
		else
		{
			CudaAABB dummy;
			pos = getPlaneCentroidPosition(*((float4*)&splitPlane), v0, v1, v2, dummy);
		}
...
__device__ __noinline__ void computePartition()
{
	...
	// Set the swap arrays
#ifndef MALLOC_SCRATCHPAD
	int* inIdx = getTriIdxPtr(s_task[threadIdx.y].dynamicMemory, triEnd-triStart, triIdxCtr);
	int* outIdx = getTriIdxPtr(s_task[threadIdx.y].dynamicMemory, triEnd-triStart, triIdxCtr+1);

	s_owner[threadIdx.y][0] = triStart;
	s_owner[threadIdx.y][1] = triEnd;
#else
	int* inIdx = getTriIdxPtr(s_task[threadIdx.y].dynamicMemory, triEnd-triStart, 0);
	int* outIdxLeft = getTriIdxPtr(s_task[threadIdx.y].dynamicMemoryLeft, 0, 0);
	int* outIdxRight = getTriIdxPtr(s_task[threadIdx.y].dynamicMemoryRight, 0, 0);
...
{
	// Data arrays
	T* x = (T*)getPPSTrisPtr(s_task[threadIdx.y].dynamicMemory, end-start);
	T* y = (T*)getPPSTrisIdxPtr(s_task[threadIdx.y].dynamicMemory, end-start);
	T* z = (T*)getSortTrisPtr(s_task[threadIdx.y].dynamicMemory, end-start);
	...
	if(step == 0) // Do local reduction, step 0
	{
		...
		if(tripos < end)
		{
			int* inIdx = getTriIdxPtr(s_task[threadIdx.y].dynamicMemory, end-start, s_task[threadIdx.y].triIdxCtr);
			int triidx = inIdx[tripos]*3;
...
__device__ __noinline__ void computeAABB()
{
	...
	T* b0 = (T*)getPPSTrisPtr(s_task[threadIdx.y].dynamicMemory, end-start);
	T* b1 = (T*)getPPSTrisIdxPtr(s_task[threadIdx.y].dynamicMemory, end-start);
	...
	do
	{
		...
		{		
			...
			{
				// Fetch triangle
				int* inIdx = getTriIdxPtr(s_task[threadIdx.y].dynamicMemory, end-start, triIdxCtr);
				int triidx = inIdx[pos]*3;
				taskFetchTri(c_bvh_in.tris, triidx, v0, v1, v2);
...
#error Unsupported SPLIT_TYPE!
		...
		case TaskType_InitMemory:
			do {
				...
				int *orig = ((int*)&g_redSplits[g_taskStackBVH.size])+s_task[threadIdx.y].popSubtask*WARP_SIZE;
...
{
	g_heapBase = (CUdeviceptr)malloc(4*numTris*sizeof(int)); // Save the base pointer (hopefully) to the heap
}
...
__global__ void MemCpyIndex(CUdeviceptr src, int size)
{
	int tid = threadIdx.x + blockIdx.x * blockDim.x;

	if(tid < size)
		((int*)g_heapBase)[tid] = ((int*)src)[tid];
}
#endif
