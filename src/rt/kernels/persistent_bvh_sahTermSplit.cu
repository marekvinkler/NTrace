...
__device__ __forceinline__ void taskSaveNodeToGMEM(int tid, int nodeIdx, const volatile CudaBVHNode& node)
{
	ASSERT_DIVERGENCE("taskSaveNodeToGMEM top", tid);
	// Copy the data to global memory
	volatile int* nodeAddr = (volatile int*)(&g_bvh[nodeIdx]);
...
__device__ __forceinline__ volatile int* getTriIdxPtr(int triIdxCtr)
{
#if SCAN_TYPE < 2
	return (volatile int*)c_bvh_in.trisIndex;
#else
	if((triIdxCtr % 2) == 0)
		return (volatile int*)c_bvh_in.trisIndex;
	else
		return (volatile int*)c_bvh_in.sortTris;
#endif
}
...
__device__ __forceinline__ void taskPrepareNext(int tid, int taskIdx, int phase)
{
	...
	else
	{
		...
		if(phase == endAfter && s_task[threadIdx.y].type != phase)
		{
#ifndef PTX_SAVE_CACHE
			g_taskStackBVH.header[taskIdx] = TaskHeader_Locked;
#else
			saveCG(&g_taskStackBVH.header[taskIdx], (int)TaskHeader_Locked;);
#endif
		}
		else
#endif
...
#else
			saveCG(&g_taskStackBVH.header[taskIdx], s_task[threadIdx.y].unfinished); // Restart this task
#endif
		}
	}
}
...
__device__ bool taskTerminationCriteria(int trisLeft, int trisRight, volatile int &subFailureCounter, volatile CudaAABB& bbox, volatile CudaAABB& bboxLeft, volatile CudaAABB& bboxRight, int& termCrit, bool& leftLeaf, bool& rightLeaf)
{
	...
	if(trisLeft+trisRight <= c_env.triLimit)
	{
	...
	if(leafCost < subdivisionCost && (trisLeft+trisRight) <= c_env.childLimit)
	{
...
/*#if SPLIT_TYPE != 0
	if(subFailureCounter > c_env.failureCount)
	{
		termCrit = TerminatedBy_FailureCounter;
		return true; // Trivial computation
	}
	
	float ratioWork = (leftCost+rightCost)/leafCost;
	if(ratioWork > 0.9f)
	{
		subFailureCounter++;
	}
...
__device__ void taskUpdateParentPtr(int tid, int parentIdx, int taskID, int newValue)
{
	...
	if(parentIdx != -1) // Not for the root
	{
		...
		//if(parentIdx >= 13000 && parentIdx <= 13200)
		//	printf("SP %d, c %d to %d\n", parentIdx, taskID, newValue);
...
__device__ bool taskDecideType(int tid, volatile TaskBVH* newTask)
{
	...
	if(taskTerminationCriteria(trisLeft, trisRight, newTask->subFailureCounter, newTask->bbox, newTask->bboxLeft, newTask->bboxRight, termCrit, leftLeaf, rightLeaf))
	{
		...
		*((int4*)&node->children) = make_int4(newTask->triStart, newTask->triEnd, newTask->parentIdx, 0);
		taskSaveNodeToGMEM(tid, newTask->nodeIdx, *node);

		if(tid == 0)
			taskUpdateParentPtr(tid, newTask->parentIdx, newTask->taskID, ~newTask->nodeIdx); // Mark this node as leaf in the hierarchy
#else
		if(tid == 0)
		{
			//printf("Left %d, right %d, depth %d\n", trisLeft, trisRight, newTask->depth);
			s_sharedData[threadIdx.y][3] = atomicAdd(&g_taskStackBVH.triTop, (trisLeft+trisRight)*3+1); // Atomically acquire leaf space, +1 is for the triangle sentinel
		}
		int triOfs = s_sharedData[threadIdx.y][3];
#ifndef WOOP_TRIANGLES
		int triIdx = createLeaf(tid, triOfs, (float*)c_bvh_in.trisOut, (int*)c_bvh_in.trisIndexOut, newTask->triStart, newTask->triEnd, (float*)c_bvh_in.tris, getTriIdxPtr(newTask->triIdxCtr)); 
#else
		int triIdx = createLeafWoop(tid, triOfs, (float4*)c_bvh_in.trisOut, (int*)c_bvh_in.trisIndexOut, newTask->triStart, newTask->triEnd, (float4*)c_bvh_in.tris, getTriIdxPtr(newTask->triIdxCtr));
#endif
		if(tid == 0)
			taskUpdateParentPtr(tid, newTask->parentIdx, newTask->taskID, triIdx); // Mark this node as leaf in the hierarchy
#endif
		...
		if(tid == 0)
		{
			...
			atomicAdd(&g_taskStackBVH.numSortedTris, newTask->triEnd - newTask->triStart);
			//printf("Split Leaf (%d, %d)\n", newTask->triStart, newTask->triEnd);
		}
#endif
#ifdef LEAF_HISTOGRAM
		if(tid == 0)
			atomicAdd(&g_taskStackBVH.leafHist[trisLeft+trisRight], 1); // Update histogram
#endif
	...
	else
	{
		...
		if(tid == 0)
		{
			...
			s_sharedData[threadIdx.y][2] = atomicAdd(&g_taskStackBVH.bvhTop, 2);
		}
		...
		int leafChunks = ((leftLeaf) ? (trisLeft*3+1) : 0) + ((rightLeaf) ? (trisRight*3+1) : 0); // Atomically acquire leaf space, +1 is for the triangle sentinel
		if(tid == 0)
		{
			if(innerNodes > 0)
				s_sharedData[threadIdx.y][2] = atomicAdd(&g_taskStackBVH.bvhTop, innerNodes); // Inner node -> create new subtasks in the final array
			if(innerNodes < 2)
		...
		int leftOfs = triOfs;
		int rightOfs = triOfs+((leftLeaf) ? (trisLeft*3+1) : 0);
#endif
		...
		if(leftLeaf)
		{
			...
			*((int4*)&node0->children) = make_int4(newTask->triStart, newTask->triRight, newTask->nodeIdx, 0);
			taskSaveNodeToGMEM(tid, childLeft, *node0);
			childLeft = ~childLeft;
#else
			// OPTIMIZE: Write out both children in one call?
#ifndef WOOP_TRIANGLES
			childLeft = createLeaf(tid, leftOfs, (float*)c_bvh_in.trisOut, (int*)c_bvh_in.trisIndexOut, newTask->triStart, newTask->triRight, (float*)c_bvh_in.tris, getTriIdxPtr(newTask->triIdxCtr));
#else
			childLeft = createLeafWoop(tid, leftOfs, (float4*)c_bvh_in.trisOut, (int*)c_bvh_in.trisIndexOut, newTask->triStart, newTask->triRight, (float4*)c_bvh_in.tris, getTriIdxPtr(newTask->triIdxCtr));
#endif
#endif
...
#ifdef LEAF_HISTOGRAM
			if(tid == 0)
				atomicAdd(&g_taskStackBVH.leafHist[trisLeft], 1); // Update histogram
#endif
		}
		...
		if(rightLeaf)
		{
			...
			*((int4*)&node1->children) = make_int4(newTask->triRight, newTask->triEnd, newTask->nodeIdx, 0);
			taskSaveNodeToGMEM(tid, childRight, *node1);
			childRight = ~childRight;
#else
			// OPTIMIZE: Write out both children in one call?
#ifndef WOOP_TRIANGLES
			childRight = createLeaf(tid, rightOfs, (float*)c_bvh_in.trisOut, (int*)c_bvh_in.trisIndexOut, newTask->triRight, newTask->triEnd, (float*)c_bvh_in.tris, getTriIdxPtr(newTask->triIdxCtr));
#else
			childRight = createLeafWoop(tid, rightOfs, (float4*)c_bvh_in.trisOut, (int*)c_bvh_in.trisIndexOut, newTask->triRight, newTask->triEnd, (float4*)c_bvh_in.tris, getTriIdxPtr(newTask->triIdxCtr));
#endif
#endif
...
#ifdef LEAF_HISTOGRAM
			if(tid == 0)
				atomicAdd(&g_taskStackBVH.leafHist[trisRight], 1); // Update histogram
#endif
		}
...
#ifndef COMPACT_LAYOUT
		*((int4*)&node->children) = make_int4(childLeft, childRight, newTask->parentIdx, 0); // Sets the leaf child pointers or the waiting status
#else
		*((int4*)&node->children) = make_int4((childLeft >= 0) ? childLeft*64 : childLeft, (childRight >= 0) ? childRight*64 : childRight, newTask->parentIdx, 0); // Sets the leaf child pointers or the waiting status
#endif
		taskSaveNodeToGMEM(tid, newTask->nodeIdx, *node);
		//if(newTask->nodeIdx < 5)
		//	printf("Node %d Children %d %d\n", newTask->nodeIdx, childLeft, childRight);
...
//#pragma unroll 2 // OPTIMIZE: Is this beneficial?
	for(int i = 0; i < 2; i++)
	{
		...
		{
			volatile int *orig = (volatile int*)&g_redSplits[g_taskStackBVH.size];
			volatile int *split = (volatile int*)&g_redSplits[beg];
		...
		g_taskStackBVH.header[beg] = newStatus; // This operation is atomic anyway
#else
		saveCG(&g_taskStackBVH.header[beg], newStatus); // This operation is atomic anyway
#endif
		//g_taskStackBVH.header[beg] = TaskHeader_Locked; // Stop the algorithm by not activating tasks
#endif
...
__device__ __noinline__ bool taskDequeue(int tid)
{
	...
	*(long long int*)&(s_sharedData[threadIdx.y][4]) = clock;
	...
	int warpIdx = (blockDim.y*blockIdx.x + threadIdx.y);
	int beg = (warpIdx % taskWarpSubtasks(*stackTop)) * WARP_SIZE - tid;
...
#else
	while(counter < WAIT_COUNT && status <= TaskHeader_Active && loadCG(&g_taskStackBVH.unfinished) < 0) // while g_taskStack is not empty and we have not found ourselves a task
#endif
	{
		// Find first active task, end if we have reached start of the array
#ifndef PTX_LOAD_CACHE
...
#else
		while(__any(beg >= 0) && __all(beg < 0 || (status = loadCG(&g_taskStackBVH.header[beg])) <= TaskHeader_Active) /*&& restartPos != tryCounter*/)
#endif
		{
			beg -= WARP_SIZE;
#ifdef COUNT_STEPS_DEQUEUE
...
#else
	saveCG(&g_taskStackBVH.header[taskIdx], (int)TaskHeader_Empty);
#endif
...
__device__ void taskFinishBinning(int tid, int taskIdx, int countDown)
{
	...
	if(s_sharedData[threadIdx.y][0] == -1)
	{
		...
		volatile ChildData* left = (volatile ChildData*)&splitArray->splits[tid].children[0];
		volatile ChildData* right = (volatile ChildData*)&splitArray->splits[tid].children[1];
		...
		{
			// Copy boxes
			left = (volatile ChildData*)&splitArray->splits[s_owner[threadIdx.y][0]].children[0];
			right = (volatile ChildData*)&splitArray->splits[s_owner[threadIdx.y][0]].children[1];

			volatile float* t_bboxLeft = (volatile float*)&(g_taskStackBVH.tasks[taskIdx].bboxLeft);
			volatile const float* g_bboxLeft = (volatile const float*)&(left->bbox);

			volatile float* t_bboxRight = (volatile float*)&(g_taskStackBVH.tasks[taskIdx].bboxRight);
			volatile const float* g_bboxRight = (volatile const float*)&(right->bbox);
			// Copy CudaAABB from corresponding task
			if(tid < sizeof(CudaAABB)/sizeof(float))
			{
				float cor;
...
#else
				t_bboxLeft[tid] = orderedIntToFloat(*(volatile int*)&g_bboxLeft[tid])+cor;
				t_bboxRight[tid] = orderedIntToFloat(*(volatile int*)&g_bboxRight[tid])+cor;
#endif
...
#else
				s_bboxLeft[tid] = orderedIntToFloat(*(volatile int*)&g_bboxLeft[tid]);
				s_bboxRight[tid] = orderedIntToFloat(*(volatile int*)&g_bboxRight[tid]);
#endif
#endif
			}
#endif
...
__device__ void taskFinishReduce(int tid, int taskIdx, int countDown)
{
	...
	if(taskCheckFinished(tid, taskIdx, countDown)) // We have finished the task and are responsible for cleaning up
	{
		...
		else // Move to PPS1
		{
			...
			SplitArray *splitArray = &(g_redSplits[taskIdx]);
			// Reduced left and right data
			volatile ChildData* left = (volatile ChildData*)&splitArray->splits[0][tid].children[0];
			volatile ChildData* right = (volatile ChildData*)&splitArray->splits[0][tid].children[1];
...
/*#ifdef SPLIT_TEST
			...
			// Copy boxes
			left = (volatile ChildData*)&splitArray->splits[0][s_sharedData[threadIdx.y][0]].children[0];
			right = (volatile ChildData*)&splitArray->splits[0][s_sharedData[threadIdx.y][0]].children[1];
...
__device__ void taskFinishSortPPS1(int tid, int taskIdx, int countDown)
{
	...
	if(taskCheckFinished(tid, taskIdx, countDown)) // We have finished the task and are responsible for cleaning up
	{
		...
		{
			int triRight = triEnd - ((volatile int*)c_bvh_in.ppsTrisBuf)[triEnd - 1]; // Must be volatile
#ifdef RAYTRI_TEST
...
__device__ void taskFinishPartition(int tid, int taskIdx, unsigned countDown)
{
	...
	if(taskCheckFinished(tid, taskIdx, countDown)) // We have finished the task and are responsible for cleaning up
	{
		...
		int triLeft = *((volatile int*)&g_taskStackBVH.tasks[taskIdx].triLeft);
		int triRight = *((volatile int*)&g_taskStackBVH.tasks[taskIdx].triRight);
...
__device__ void taskFinishAABB(int tid, int taskIdx, unsigned countDown)
{
	...
	{
		// Data arrays
		volatile float* x = (volatile float*)c_bvh_in.ppsTrisBuf;
		volatile float* y = (volatile float*)c_bvh_in.ppsTrisIndex;
		volatile float* z = (volatile float*)c_bvh_in.sortTris;
...
__device__ void trianglesPlanePosition(int triStart, int triEnd, const float4& plane, int& tb, int& tf)
{
	...
	{
		volatile int* inIdx = getTriIdxPtr(s_task[threadIdx.y].triIdxCtr);
...
{	
	...
	{
		// Update the bins
		volatile SplitRed *split = (volatile SplitRed*)&(g_redSplits[taskIdx].splits[warpIdx][planePos]);
		...
		//{
		//	volatile int* g_split = (volatile int*)split;
...
__device__ void binTrianglesAtomic(int tid, int subtaskFirst, int subtaskLast, int taskIdx, int triStart, int triEnd, const volatile CudaAABB& bbox, int axis)
{
	if(tid < PLANE_COUNT)
	{
		...
		for(;triPos < triLast; triPos++)
		{
			...
			volatile float *global = &((volatile float*)c_bvh_in.tris)[triIdx*4] + tid;
			*shared = *global;
			}
			__threadfence_block();*/
...
__device__ int classifyTri(int tid, int subtask, int start, int end, volatile const float4& splitPlane)
{
	...
	if(tripos < end)
	{
		//int triidx = ((volatile int*)c_bvh_in.trisIndex)[tripos]*3;
...
__device__ __noinline__ void computePPS()
{
	...
	do
	{
		...
		pps<int>(threadIdx.x, popSubtask, step, c_bvh_in.ppsTrisIndex, c_bvh_in.ppsTrisBuf, s_sharedData[threadIdx.y], triStart, triEnd, 1);
		subtasksDone = taskReduceSubtask(popSubtask, popStart, popCount);
	} while(subtasksDone == -1);
...
__device__ __noinline__ void computePPSUp()
{
	...
	do
	{
		...
		scanUp<int>(threadIdx.x, popSubtask, step, (volatile int*)c_bvh_in.ppsTrisBuf, (volatile int*)c_bvh_in.ppsTrisIndex, s_sharedData[threadIdx.y], triStart, triEnd, 1, plus, 0);
		subtasksDone = taskReduceSubtask(popSubtask, popStart, popCount);
	} while(subtasksDone == -1);
...
__device__ __noinline__ void computePPSDown()
{
	...
	do
	{
		scanDown<int>(threadIdx.x, popSubtask, step, (volatile int*)c_bvh_in.ppsTrisBuf, (volatile int*)c_bvh_in.ppsTrisIndex, s_sharedData[threadIdx.y], triStart, triEnd, 1, plus, 0);
		subtasksDone = taskReduceSubtask(popSubtask, popStart, popCount);
	} while(subtasksDone == -1);
...
__device__ __noinline__ void computeSort()
{
	...
	do
	{
		sort(threadIdx.x, popSubtask, step, c_bvh_in.ppsTrisIndex, c_bvh_in.ppsTrisBuf, c_bvh_in.sortTris, c_bvh_in.trisIndex, triStart, triEnd, triRight, 1, false);
		subtasksDone = taskReduceSubtask(popSubtask, popStart, popCount);
	} while(subtasksDone == -1);
...
__device__ __noinline__ void computePartition()
{
	...
	// Set the swap arrays
	volatile int* inIdx = getTriIdxPtr(triIdxCtr);
	volatile int* outIdx = getTriIdxPtr(triIdxCtr+1);
...
{
	// Data arrays
	volatile T* x = (volatile T*)c_bvh_in.ppsTrisBuf;
	volatile T* y = (volatile T*)c_bvh_in.ppsTrisIndex;
	volatile T* z = (volatile T*)c_bvh_in.sortTris;
...
__device__ void computeAABB(int tid, int taskIdx, int subtask, int step, int start, int right, int end, T(*op)(T,T), T identity, T epsilon)
{
	...
	if(pos < end)
	{
		...
		if(step == 0)
		{
			...
			red[tid] = op(op(v0.x, v1.x), v2.x)+epsilon; // OPTIMIZE: Do triangle bounding box computation once and load only the result
			//int triidx = ((volatile int*)c_bvh_in.trisIndex)[pos];
			//if(s_task[threadIdx.y].type == TaskType_AABB_Min)
...
__device__ __noinline__ void computeAABB()
{
	...
	volatile T* b0 = (volatile T*)c_bvh_in.ppsTrisBuf;
	volatile T* b1 = (volatile T*)c_bvh_in.ppsTrisIndex;

	// Array for holding partial results caused by segmented reduction - one box (right-end interval)
	volatile T* st = (volatile T*)c_bvh_in.ppsTrisIndex; // Use first item of the input/output array
...
__device__ void reduceBins(int tid, int subtask, int taskIdx, int step)
{
	...
	if(step == 0) // Do local reduction, step 0
	{
		...
		volatile ChildData* data = (volatile ChildData*)&splitArray->splits[warpIdx][split].children[child];
		volatile ChildData* out = (volatile ChildData*)&splitArray->splits[warpBlock * WARP_SIZE][split].children[child];
	...
	else // Do global reduction, step 1 to n
	{
		...
		volatile ChildData* data = (volatile ChildData*)&splitArray->splits[posNext][split].children[child];
		volatile ChildData* out = (volatile ChildData*)&splitArray->splits[posThis][split].children[child];
...
__device__ __noinline__ void computeObjectSplitTree()
{
	...
	while(__any(!isLeaf)) // While there is a segment that is not yet a leaf
	{
		...
		bool leftLeaf = isLeaf || (bestPos-segmentStart <= c_env.triLimit);
		bool rightLeaf = isLeaf || (segmentEnd-bestPos <= c_env.triLimit);
#ifdef SAH_TERMINATION
		float leafCost, leftCost, rightCost, leftCount, rightCount;
		...
		if(leafCost < subdivisionCost && (leftCount+rightCount) <= c_env.childLimit)
		{
			leftLeaf = true;
			rightLeaf = true;
			/*isLeaf = true;
		...
		if(!isLeaf && tid == segmentStart)
			childrenCount = (leftLeaf ? 0 : 1) + (rightLeaf ? 0 : 1); // Count the number of segments in the next iteration
#endif

		int nodePosition = childrenCount;
		scanWarp(tid, nodePosition, plus);
		...
		{
			// Inner node -> create new subtasks in the final array
			nodeOffset = atomicAdd(&g_taskStackBVH.bvhTop, nodeCount);
		}
		...
		if(!isLeaf)
		{
...
#ifndef COMPACT_LAYOUT
			// Check if children should be leaves
			if(tid == segmentStart && leftLeaf) // Only the first thread of the segment may write the leafs because it may be the only thread in the segment
			{
				// Leaf -> same bounding boxes
			...
			if(tid == segmentStart && rightLeaf) // Only the first thread of the segment may write the leafs because it may be the only thread in the segment
			{
				// Leaf -> same bounding boxes
	...
	volatile int* outIdx = getTriIdxPtr(s_task[threadIdx.y].triIdxCtr);
	...
	if(tid < end && tid == segmentStart)
	{
		...
		taskUpdateParentPtr(tid, parentIdx, taskID, ~triPosition); // Mark this node as leaf in the hierarchy
#ifdef LEAF_HISTOGRAM
	...
	while(s_task[threadIdx.y].lock != LockType_Free || taskDequeue(tid)) // Main loop of the programme, while there is some task to do
	{
		...
		*(long long int*)&(s_sharedData[threadIdx.y][4]) = 0;
		...
		// Copy first cache line of task to shared memory
#ifdef DIVERGENCE_TEST
			if(s_task[threadIdx.y].popTaskIdx >= 0 && s_task[threadIdx.y].popTaskIdx < g_taskStackBVH.size)
			{
				//taskLoadFirstFromGMEM(tid, s_task[threadIdx.y].popTaskIdx, &s_task[threadIdx.y]);
				volatile TaskBVH* g_task = &g_taskStackBVH.tasks[s_task[threadIdx.y].popTaskIdx];
				taskAddr[tid] = ((volatile int*)g_task)[tid]; // Every thread copies one word of task data
#ifdef DEBUG_INFO
				int offset = 128/sizeof(int); // 128B offset
				taskAddr[tid+offset] = ((volatile int*)g_task)[tid+offset]; // Every thread copies one word of task data
			...
			taskAddr[tid] = loadCG(&((int*)g_task)[tid]); // Every thread copies one word of task data
#endif
#ifdef DEBUG_INFO
			int offset = 128/sizeof(int); // 128B offset
			if(tid < TASK_GLOBAL) // Prevent overwriting local data saved in task
...
#error Unsupported SPLIT_TYPE!
		...
		case TaskType_InitMemory:
			do {
				...
				volatile int *orig = ((volatile int*)&g_redSplits[g_taskStackBVH.size])+s_task[threadIdx.y].popSubtask*WARP_SIZE;
				volatile int *split = ((volatile int*)&g_redSplits[s_task[threadIdx.y].popTaskIdx])+s_task[threadIdx.y].popSubtask*WARP_SIZE;
				split[tid] = orig[tid]; // Each thread copies 1 int-sized variable
...
#error Unknown SCAN_TYPE!
		...
		case TaskType_RayTriTestSORT1:
			do {
				...
				if(triidx < s_task[threadIdx.y].triRight && ((volatile int*)c_bvh_in.ppsTrisIndex)[triidx] > 0)
					printf("Tri error should be -1/0 is %d! Start %d, Left %d, Right %d, End %d\n", ((volatile int*)c_bvh_in.ppsTrisIndex)[triidx], s_task[threadIdx.y].triStart, s_task[threadIdx.y].triLeft, s_task[threadIdx.y].triRight, s_task[threadIdx.y].triEnd);

				if(triidx >= s_task[threadIdx.y].triRight && triidx < s_task[threadIdx.y].triEnd && ((volatile int*)c_bvh_in.ppsTrisIndex)[triidx] < 1)
